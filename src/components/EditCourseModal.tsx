...
export function EditCourseModal({ isOpen, onClose, onSuccess, course }: EditCourseModalProps) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      let videoUrl = course.promo_video_url;
      ...
      const { error: dbError } = await supabase.from('courses').update({
        title,
        description,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <div className="p-8 md:p-12">
              ...
              <form onSubmit={step === 1 ? (e) => { e.preventDefault(); setStep(2); } : handleUpdate} className="space-y-8">
                {step === 1 ? (
                  ...
                  <div className="space-y-6">
                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">Update Video (Optional)</label>
                        <div className={`relative border-2 border-dashed rounded-[2rem] p-12 transition-all text-center group ${
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                               <p className="text-white font-bold">{videoFile ? videoFile.name : 'Keep current or upload new'}</p>
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        {step === 1 ? <span>Next Step</span> : <span>Update Course</span>}
                      </>
                    )}
                  </button>
                </div>
